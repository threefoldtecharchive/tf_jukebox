...
from jumpscale.packages.jukebox.models import State, BlockchainNode
...
class JukeboxDeployment(Base):
    ...
    nodes_count = fields.Integer()
    pool_id = fields.List(fields.Integer()) # in case we will support multiple locations
    ...
    def calculate_payment_from_container_resources(
        self, cpu, memory, disk_size, duration, farm_id=None, farm_name="freefarm", disk_type=DiskType.SSD
    ):
    ...
    def calculate_required_units(self, cpu, memory, disk_size, duration_seconds, number_of_containers=1):
        cont = Container()
    ...
    def get_possible_farms(self, cru, sru, mru, number_of_deployments):
    ...
    def show_payment(self, bot, cost, wallet_name, expiry=5, description=None):
    ...
    def create_capacity_pool(self, wallet, cu=100, su=100, ipv4us=0, farm="freefarm", identity_name=None):
        zos = j.sals.zos.get(identity_name)
    ...
    def get_container_ip(self, network_name, identity_name, node, pool_id, tname, excluded_ips=None):
    ...
    def deploy_network(self, identity_name, pool_id, network_name, owner_tname, ip_range=None):
    ...
    def calculate_funding_amount(self, identity_name):
        identity = j.core.identity.find(identity_name)
        if not identity:
            return 0
        total_price = 0
    ...
    def get_wallet_funding_info(self, identity_name):
